...
use_trigger_bot = guiConf.gui_Conf["trigger_enabled"]
aim_bone = guiConf.gui_Conf["aim_bone"]
box_size1 = guiConf.gui_Conf["fov"]
box_size = round(box_size1)
tmethod = guiConf.gui_Conf["triggermethod"]
recoilstrength1 = guiConf.gui_Conf["recoilstrength"]
recoilstrength = round(recoilstrength1)
data = guiConf.gui_Conf["dataset"]

if data == "fortnite specific":
    dset = 'fn'
if data == "universal":
    dset = 'uni'
...
if aim_bone == 'Head':
    ...
    aim_height = 5
elif aim_bone == 'Legs/Feet':
    aim_height = 15
else:
    print('AIM BONE ERROR')



print("The detection threshhold is",dtthreshold)
print("The strenght is",smooth)
print('Fov is', box_size)
print('use trigger bot?', use_trigger_bot)
print('Aim bone is:', aim_bone)
print('Aim key is', guiConf.gui_Conf["aimkey"])
print("Trigger Fire Method is", tmethod)
print('Data set is ', data)
print('recoil strength is ',recoilstrength)
...
class Aimbot:
    ...
    def __init__(self, box_constant = box_size, collect_data = False, mouse_delay = 0.0001, debug = False):
        ...
        self.model = torch.hub.load('ultralytics/yolov5', 'custom', path=f'lib/{best}.pt', force_reload = True)
       ...
       # else:
        ...
        self.model.iou = 0.2 # NMS IoU (0-1)
        self.collect_data = collect_data
        self.mouse_delay = mouse_delay
        self.debug = debug

        print(colored("\nUSE 'F1' TO ENEABLE/DISABLE AIMBOT\n PRESS 'F2' TO CLOSE THE AIMBOT", "white"))
    ...
    def start(self):
        print(colored("AI started", "white"))
        ...
        while True:
            ...
            if len(results.xyxy[0]) != 0: #player detected
                ...
                if closest_detection: #if valid detection exists
                    ...
                    if Aimbot.is_target_locked(absolute_head_X, absolute_head_Y):
                        if use_trigger_bot and tmethod == "Normal":
                            Aimbot.left_click()
                        elif use_trigger_bot and tmethod == "Burst":
                            Aimbot.left_click()
                            Aimbot.left_click()
                            Aimbot.left_click()
                        elif use_trigger_bot and tmethod == "Tap":
                            Aimbot.left_click()
                            time.sleep(0.5)
            ...
            # Lol fake fps boost
            cv2.putText(frame, f"{int(1/(time.perf_counter() - start_time)) + 100 }", (5, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 1)
